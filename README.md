...
| (규칙 밖) 낱말 강조 | 모델이 `*이렇게*` 감싸면 `parseSpans`가 조각으로 쪼갠다 |
...
한 줄 안에서 일부만 다른 색으로 칠할 수 있습니다. 모델이 `국제회의 *47건*을`
처럼 별표로 감싸면 `cardnews.ts`의 `parseSpans`가 조각(`TextSpan`)으로 쪼개고,
템플릿이 `accent: true`인 조각만 포인트 컬러로 그립니다.

모델에게 중첩 객체 배열을 받는 대신 문자열 안에 표시를 넣게 했습니다. 응답
형식이 문자열 배열 그대로라 **이미 저장해 둔 결과도 그대로 읽힙니다.** 모델
입장에서도 배열을 겹쳐 만드는 것보다 쉽습니다.
